...
extern void appcmbs_ObjectSignal(char * psz_Info, int n_InfoLen, int n_Info, int n_Event);
...
int app_OnTargetListChangeNotif(void * pv_AppRef, void * pv_EventData)
{
    ...
    switch (st_Notif.e_ListId)
    {
        case CMBS_LA_LIST_SUPPORTED_LISTS:
            printf("List Of Supported Lists");
            break;

        case CMBS_LA_LIST_INTERNAL_NAMES:
            printf("Internal Names List");
            break;

        case CMBS_LA_LIST_DECT_SETTINGS:
            printf("DECT System settings list");
            break;
